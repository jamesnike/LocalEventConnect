...
- July 13, 2025. OPTIMIZED MESSAGES TAB PERFORMANCE: Enhanced group chat query caching to prevent unnecessary reordering when clicking My Events tab. Changed staleTime from 0 to 5 minutes and refetchInterval from 10 seconds to 60 seconds. Modified background refresh logic to only trigger when cache is empty, eliminating excessive API calls and improving user experience.
- July 13, 2025. FIXED PRIVATE CHAT DUPLICATION: Resolved duplicate private chat entries in Messages tab by excluding private chats from getUserEventIds function and handling them separately in notification system. Private chats now appear only once while maintaining proper notification functionality.
