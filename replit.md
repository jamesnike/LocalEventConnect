...
- July 11, 2025. COMPLETELY RESOLVED SYSTEM-WIDE 404 ERRORS DURING SCROLLING: Successfully fixed critical authentication state management issue causing 404 errors when scrolling on any page. Root cause was complex: 1) useAuth hook configured with `refetchOnReconnect: false` causing authentication state loss during Vite reconnections, 2) Infinite React loop in EventContentCard component caused by problematic useEffect dependencies. Fixed by: enabling `refetchOnReconnect: true` in useAuth hook, removing problematic useEffect hooks that were calling setState repeatedly, implementing cleaner message management using useMemo instead of useState to combine API and WebSocket messages without causing infinite renders. System now maintains stable authentication state and proper component lifecycle management.
