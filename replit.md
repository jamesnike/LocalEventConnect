...
- July 11, 2025. FIXED SYSTEM-WIDE 404 ERRORS DURING SCROLLING: Resolved critical authentication state management issue causing 404 errors when scrolling on any page. Root cause was useAuth hook configured with `refetchOnReconnect: false`, which caused authentication state to be lost when Vite reconnected during development. Fixed by enabling `refetchOnReconnect: true` to maintain authentication state across Vite reconnections. Also fixed infinite React loop in EventContentCard component by removing markEventAsRead from useEffect dependencies.
