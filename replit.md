...
- July 11, 2025. OPTIMIZED EVENTDETAIL BROWSE PAGE DATA FETCHING: Enhanced EventDetail component to only fetch essential data when opened from Browse page. Instead of fetching complete event data, component now makes separate targeted API calls for RSVP status (/api/events/:id/rsvp-status) and Members data (/api/events/:id/attendees) only when fromPage === 'browse'. This reduces unnecessary API overhead while maintaining fresh data accuracy for critical user interactions. Non-browse page usage continues to use existing queries for full functionality.
