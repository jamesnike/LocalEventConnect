...
- July 11, 2025. REDESIGNED TIME FILTER LAYOUT: Completely redesigned CategoryFilter component to use 3 rows with smaller buttons for better organization. Changed from 2-row alternating pattern to 3-row column layout where AM, PM, and Night times for each day are vertically aligned in the same column. Made buttons smaller (reduced padding, text size, and icon size) and tighter spacing for more compact display. Layout now shows: Row 1 (All AM times), Row 2 (All PM times), Row 3 (All Night times).
